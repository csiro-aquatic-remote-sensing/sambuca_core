...
class TestSubstrateLoading(object):
    def test_load_envi_spectral_library(self):
        directory = resource_filename(sbc.__name__, "tests/data/substrates")
        base_name = "HI_3"

        loaded_substrates = sbc.load_envi_spectral_library(directory, base_name)

        expected_spectra = [
            spectra_name(base_name.lower(), x)
            for x in ["Acropora", "sand", "Turf Algae"]
        ]
    ...
    def test_load_whole_directory(self):
        ...
        expected_names = [
            spectra_name("moreton_bay_speclib_final", x) for x in mb_names
        ]
        for expected_name in expected_names:
            assert expected_name in loaded_substrates
    ...
    def test_load_excel(self):
        filename = resource_filename(
            sbc.__name__, "tests/data/substrates/Moreton_Bay_speclib.xlsx"
        )
    ...
    def test_load_csv(self):
        filename = resource_filename(
            sbc.__name__, "tests/data/substrates/Moreton_Bay_speclib.csv"
        )
...
class TestMagicFileDetectionLoading(object):
    def test_csv(self):
        filename = resource_filename(
            sbc.__name__, "tests/data/siop/aw_340_900_lw2002_1nm.csv"
        )
    ...
    def test_envi_hdr_extension(self):
        filename = resource_filename(sbc.__name__, "tests/data/siop/WL_aphy_1nm.hdr")
        data = sbc.load_spectral_library(filename)
        assert isinstance(data, dict)
        assert len(data) == 1

    def test_missing(self):
        filename = resource_filename(sbc.__name__, "tests/data/siop/missing_file")
...
class TestValidationFlag(object):
    def test_invalid_file_fails(self):
        filename = resource_filename(
            sbc.__name__, "tests/data/nedr/fails_validation.hdr"
        )
