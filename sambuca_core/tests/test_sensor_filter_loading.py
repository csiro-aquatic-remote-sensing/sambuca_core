...
class TestExcelSensorFilterLoading(object):
    ...
    def test_xls_format(self):
        file = resource_filename(
            sbc.__name__, "tests/data/sensor_filters/sensor_filters.xls"
        )
    ...
    def test_unknown_worksheet_doesnt_throw(self):
        ...
        sbc.load_sensor_filters_excel(file, sheet_names=["non_existant"])
    ...
    def test_load_single_worksheet(self):
        ...
        loaded_filters = sbc.load_sensor_filters_excel(
            file, normalise=False, sheet_names=[expected_name]
        )
    ...
    def test_load_multiple_skips_invalid_and_missing_sheets(self):
        ...
        loaded_filters = sbc.load_sensor_filters_excel(
            file, normalise=False, sheet_names=names
        )
    ...
    def test_normalise(self):
        ...
        expected_name = "3_band_350_900"
        loaded_filters = sbc.load_sensor_filters_excel(
            file, normalise=True, sheet_names=[expected_name]
        )
    ...
    def test_wavelengths(self):
        ...
        loaded_filters = sbc.load_sensor_filters_excel(
            file, normalise=False, sheet_names=expected_names
        )
...
class TestSpectralLibrarySensorFilterLoading(object):
    def test_valid_load_casi(self):
        directory = resource_filename(sbc.__name__, "tests/data/sensor_filters")
        base_name = "CASI04_350_900_1nm"

        wavelengths, sensor_filter = sbc.load_sensor_filter_spectral_library(
            directory, base_name, normalise=False
        )
...
class TestLoadAllSensorFilters(object):
    def test_invalid_directory(self):
        directory = resource_filename(sbc.__name__, "tests/data/missing_stuff")
    ...
    def load_all_filters(self):
        ...
        return sbc.load_sensor_filters(
            directory,
            normalise=False,
            spectral_library_name_parser=lambda path: splitext(basename(path).lower())[
                0
            ].split("_")[0],
        )
